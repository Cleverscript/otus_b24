...
abstract class AbstractIblockPropertyValuesTable extends DataManager
{
    ...
    {
        return 'b_iblock_element_prop_s'.static::$iblockEntityId;
    }
    ...
    {
        return 'b_iblock_element_prop_m'.static::$iblockEntityId;
    }
    ...
    public static function getMap(): array
    {
        ...
        $cacheDir = 'iblock_property_map/'.static::$iblockEntityId;
    ...
    public static function add(array $data)
    {
        ...
        $fields = [
            'NAME'            => $data['NAME'],
            'IBLOCK_ID'       => static::$iblockEntityId,
    ...
    {
        if (isset(static::$properties[static::$iblockEntityId])) {
            return static::$properties[static::$iblockEntityId];
        }

        $dbResult = PropertyTable::query()
            ->setSelect(['ID', 'CODE', 'PROPERTY_TYPE', 'MULTIPLE', 'NAME', 'USER_TYPE'])
            ->where('IBLOCK_ID', static::$iblockEntityId)
            ->exec();
        while ($row = $dbResult->fetch()) {
            static::$properties[static::$iblockEntityId][$row['CODE']] = $row;
        }
    ...
    {
        $iblockId = $iblockId ?: static::$iblockEntityId;
        if (empty($iblockId)) {
            return;
        }
    ...
    public static function getEnumPropertyOptions(string $propertyCode, string $byKey = 'ID'): array
    {
        $dbResult = PropertyEnumerationTable::getList([
            ...
            'filter' => ['=PROPERTY.CODE' => $propertyCode, 'PROPERTY.IBLOCK_ID' => static::$iblockEntityId],
        ]);
        while ($row = $dbResult->fetch()) {
